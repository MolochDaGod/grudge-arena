...
/** Master-armor `slotType` → our canonical Equipment slot key. */
const ARMOR_SLOT_MAP = Object.freeze({
  helm: "head", head: "head", hood: "head", crown: "head",
  neck: "neck", amulet: "neck",
  shoulders: "shoulders", shoulder: "shoulders", pauldrons: "shoulders",
  back: "back", cloak: "back", cape: "back",
  chest: "chest", body: "chest", robe: "chest", tunic: "chest",
  wrists: "wrists", bracer: "wrists", bracers: "wrists",
  hands: "hands", gloves: "hands", gauntlets: "hands",
  waist: "waist", belt: "waist", sash: "waist",
  legs: "legs", pants: "legs", greaves: "legs",
  feet: "feet", boots: "feet", sabatons: "feet",
});

/** Map an item catalog entry to its equipment slot.
 *  Handles both the canonical master-items schema (type/slotType/subCategory)
 *  and the legacy items-database schema (category/tooltip/name guesses).
 */
export function inferEquipmentSlot(catalogItem) {
  if (!catalogItem) return null;
  const type = (catalogItem.type || "").toLowerCase();
  const cat = (catalogItem.category || "").toLowerCase();
  const sub = (catalogItem.subCategory || "").toLowerCase();
  const slotType = (catalogItem.slotType || "").toLowerCase();

  // ── Weapons ──────────────────────────────────────────────────────
  if (type === "weapon" || cat === "weapon" || /^(swords|axes|spears|bows|staves|staffs|wands|sabres|scythes|runeblades|rifles|crossbows|maces|hammers|daggers)$/.test(cat)) {
    if (sub === "longbow" || sub === "rifle" || sub === "crossbow") return "ranged";
    if (cat === "bows" || cat === "rifles" || cat === "crossbows") return "ranged";
    return "mainHand";
  }

  // ── Off-hands ────────────────────────────────────────────────────
  if (type === "offhand-tome" || cat === "offhand" || cat === "tome" || cat === "shield" || cat === "focus" || cat === "relic") {
    return "offHand";
  }

  // ── Rings / Trinkets ─────────────────────────────────────────────
  if (cat === "ring" || type === "ring") return "ring1";
  if (cat === "trinket" || type === "trinket" || type === "artifact") return "trinket1";

  // ── Armor ────────────────────────────────────────────────────────
  if (type === "armor" || cat === "armor") {
    // Master schema: slotType is "Helm", "Chest", "Legs", etc.
    if (slotType && ARMOR_SLOT_MAP[slotType]) return ARMOR_SLOT_MAP[slotType];
    // Legacy fallback: name hints.
    const name = (catalogItem.name || catalogItem.baseName || "").toLowerCase();
    for (const [needle, slot] of Object.entries(ARMOR_SLOT_MAP)) {
      if (name.includes(needle)) return slot;
    }
    return "chest";
  }

  // Final fallback: if a tooltip hint exists, use it.
