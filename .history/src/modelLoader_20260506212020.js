...
export async function createHeroUnit(hero, weaponOverride = null, opts = {}) {
  const weaponType = (weaponOverride && hero.weapons.includes(weaponOverride))
    ? weaponOverride
    : hero.defaultWeapon;
  ...
  try {
    ...
    console.warn(`[modelLoader] ${hero.id}: pack model not found (${hero.modelPath}), using race fallback`);
  ...
  console.log(`[modelLoader] Hero ${hero.displayName} (${hero.id}) ready: ${controller.actions.size} anims (${bareRegistered} bare-aliased from '${animClass}'), weapon: ${weaponType}, tier: ${tierCfg.name}`);
  return { scene, mixer, controller, raceConfig, resolvedWeapon: weaponType, tier, hero, race: hero.race };
}
...
export function swapWeapon(unit, newWeaponType) {
  ...
  scene.traverse(node => {
    ...
    const toRemove = node.children.filter(c => c.name === '__weapon' || c.name === '__shield');
    toRemove.forEach(c => node.remove(c));
  });

  // Attach new weapon
  const newWeapon = createWeaponMesh(newWeaponType);
  tintWeaponMesh(newWeapon, raceConfig.gearTint, factionColors.emissive, tierCfg);
  ...
  console.log(`[modelLoader] Weapon swapped → ${newWeaponType} (${newAnimClass})`);
}
